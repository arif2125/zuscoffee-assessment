    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="pgender">Edit</li>
                        <li class="breadcrumb-item active" aria-current="pgender">{{ $route.params.id }}</li>
                    </ol>
        </nav>
        <div class="col-lg-6 ">
            <!-- <router-link :to="{ name : 'List' }" class="dropdown-toggle" > -->
              <a href="javascript:void(0)" @click="$router.back()" style="color : #4361ee">Back</a>
            <!-- </router-link> -->
           </div>
...
const  getData = async () => {
    ...
    await axios.get('/api/user/edit/' + route.params.id ).then(({data})=> {
        ...
        form.age = user.detail.age
        form.gender = user.detail.gender

        let roles = user.roles;
        if(roles){
            roles.forEach(element => {
                    form.roles.push(element.id);
            });
        }
...
const submit = async () => {
        ...
        await axios.get('/sanctum/csrf-cookie')
        await axios.put('/api/user/update', form).then(({data})=> {
